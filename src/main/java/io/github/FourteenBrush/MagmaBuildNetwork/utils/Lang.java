...
public enum Lang {
    ...
    MAINTENANCE_ALREADY_ENABLED("maintenance.already-enabled", "&cServer maintenance is already enabled!"),
    MAINTENANCE_ALREADY_DISABLED("maintenance.already-disabled", "&cServer maintenance is already disabled!"),
    MAINTENANCE_ENABLED("maintenance.enabled", "&6Server maintenance has been enabled, kicked all players that don't have permission to stay."),
    MAINTENANCE_DISABLED("maintenance.disabled", "&6Server maintenance has been disabled."),
    ...
    CHANNEL_JOIN_NOT_PERMITTED("chat.channel-join-not-permitted", "&cYou don't have permission to join {0}&c!"),
    ...
    Lang(String path, String fallback) {
        ...
        if (real == null || real.isEmpty()) {
            Utils.logWarning("Missing lang data found: " + path + ", using fallback");
            real = fallback;
        }
