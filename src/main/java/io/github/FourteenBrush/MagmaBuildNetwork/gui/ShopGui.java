...
public class ShopGui extends GuiCreator {
    ...
    private void openMenu(Material material, double price, boolean sell) {
        ...
        setItem(29, createItem(sell ? Material.RED_STAINED_GLASS_PANE : Material.GREEN_STAINED_GLASS_PANE, sell ? "&aSell" : "&aBuy", null),
                player -> sell(sell, material, item.getAmount(), price));
    ...
    private void sell(boolean sell, Material material, int amount, double price) {
        ...
        PlayerUtils.message(player, Utils.colorize((sell ? "&a+&b " : "&c-&b ") + df.format(amount * price) + " &3coins"));
    }
