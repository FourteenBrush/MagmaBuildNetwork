...
public class PlayerListener implements Listener {
    ...
    public void onJoin(PlayerJoinEvent event) {
        ...
        if (player.getPersistentDataContainer().getOrDefault(Keys.VANISHED, PersistentDataType.BYTE, (byte)0) == 1) {
            CommandVanish.getInstance().vanish(player, true);
            event.setJoinMessage(null);
        } else event.setJoinMessage(Lang.JOIN_MESSAGE.get(player.getName()));
        plugin.getPlayerManager().getUser(player.getUniqueId()).sendActionbar();
        Logger.DEBUG.log(player.getPersistentDataContainer().getKeys().toString()); // todo
    ...
    public void onQuit(PlayerQuitEvent event) {
        Player player = event.getPlayer();
        plugin.getPlayerManager().getUser(player.getUniqueId()).logoutSafely();
        plugin.removeUserFromCache(player.getUniqueId());
        GuiCreator.getOpenInventories().remove(player.getUniqueId());
        if (CommandVanish.getVanishedPlayers().remove(player.getUniqueId())) {
